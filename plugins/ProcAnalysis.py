...
check_proc = {"systemd": "init", "udevadm": "systemd-udevd", "bash": "-bash", "dash": "sh", "busybox": "sh", "node": "next-server"}
...
class ProcAnalysis:
            ...
            for line in result['result'].splitlines()[1:]:
                try:
                    ...
                    if ':' in exe:
                        ...
                        exe = exe[1:-1]
                    self.ps[pid] = { 'user': parts[0], 'cpu': float(parts[2]), 'mem': float(parts[3]), 'tty': parts[6], 'time': parts[9], 'command': ' '.join(parts[10:]), 'exe': exe }
                except:
                    pass
    ...
    def check_pid(self):
        ...
        self.pids = [int(file) for file in os.listdir('/proc') if file.isdigit()]
        pid_self = int(os.readlink('/proc/self'))

        for pid in self.pids:
            if pid not in self.ps and pid not in range(pid_self - 2, pid_self + 2):
                output += f'PID: {pid:<8}\t path:/proc/{pid:<8}\t[!] {get_color("隐藏 pid")}\n'

        get_output(info, output)

    def check_exe(self):
        info = get_color('命令替换排查', 'green') + '\n可能会有部分误判，可以在 plugins/ProcAnalysis.py 自定义添加规则 "true_exe" : "exe"'
        output = ''

        if self.ps.keys():
            for pid in self.pids:
                try:
                    true_exe = os.path.basename(os.readlink(f'/proc/{pid}/exe')).split()[0]
                    exe = self.ps[pid]['exe']
                    if true_exe and (true_exe != exe) and (((true_exe in check_proc) and (exe not in check_proc[true_exe])) or (true_exe not in check_proc)):
    ...
    def check_mount(self):
        ...
        if os.path.exists('/proc/mounts'):
            try:
                for pid in re.findall(r'/proc/(\d+)', open('/proc/mounts').read()):
