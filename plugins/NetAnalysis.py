...
class NetAnalysis:
    ...
    def get_localhost(self):
        result = exec_command('ip -4 addr show')
            ...
            for line in result['result'].splitlines()[1:]:
                try:
                    ...
                    if remote_addr not in self.ip_list and remote_port != "*":
                        output.append(f'local :{local:<20}\tremote :{remote:<20}\tpid :{pid_program:<40}\t[!] {get_color("发现远程连接", "red")}')
    ...
    def check_eth(self):
        ...
        output = ''
        try:
            output = [f'网卡: {line.strip():<50}\t[!] {get_color("网卡检测")}' for line in os.listdir('/sys/class/net')]
        except:
            pass
        get_output(info, "\n".join(output))
